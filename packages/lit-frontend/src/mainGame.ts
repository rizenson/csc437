...
export class MainGame extends LitElement {
  ...
  darkMode() {
    ...
    if (darkMode && darkMode == "true" ? true : false) {
      (document.querySelector(":root") as HTMLInputElement).style!.setProperty(
        "--primary-color",
        "white"
      );
      (document.querySelector(":root") as HTMLInputElement).style!.setProperty(
  ...
  getCombinations(arr: any, min: number, max: number) {
    return [...Array(max).keys()]
      .reduce(
        (result) =>
          arr.concat(
            result.flatMap((val) => arr.map((char: any) => val + " " + char))
          ),
        []
      )
      .filter((val: any) => val.length >= min);
  }
  ...
  render() {
    ...
    const renderButtons = (index: number) => {
      return html`
        ...
        >
          <img
            src="images/cards/${this.numberOfSymbols}_${imageStyle
              ? imageStyle
              : "dot"}_${this.cardsToShow.length > index
