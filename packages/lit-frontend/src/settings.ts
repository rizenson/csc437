...
export class SettingsBox extends LitElement {
  ...
  show() {
    ...
    (document
      .getElementById("settings-box")!!
      .shadowRoot!!.firstChild!!.nextSibling as HTMLElement).classList.add("show");
    if (this.refresh) {
      location.reload();
    }
  }
  ...
  toggleHardMode() {
    ...
    if (this.hardMode == true) {
      ...
      for (let j = 0; j < cards.length; j++) {
        ...
        if (rand == 1) {
          (cards[j] as HTMLElement).style!.transform = "scaleX(-1)";
        } else if (rand == 2) {
          (cards[j] as HTMLElement).style!.transform = "scaleY(-1)";
        } else if (rand == 3) {
          (cards[j] as HTMLElement).style!.transform = "scale(-1)";
        } else {
          (cards[j] as HTMLElement).style!.transform = "scale(1)";
        }
      }
      ...
      for (var card in cards) {
        (cards[(card as any)] as HTMLElement).style!.transform = "scale(1)";
      }
    }
  }

  colors(num: number, event: any) {
